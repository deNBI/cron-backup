...
SIZE_TYPE="GiB"
SIZE_TYPE_OPTIONS=(MB MiB GB GiB TB TiB)
MAX_SIZE=2
CUT_SIZE=1
...
if [ $CURRENT_SIZE_BYTES -gt $MAX_SIZE_BYTES ]; then
  ...
  while [ $CURRENT_SIZE_BYTES -gt $CUT_SIZE_BYTES ]; do
    echo "Maximum Size $MAX_SIZE exceeded - Current Size  $CURRENT_SIZE_BYTES"
    CURRENT_SIZE_BYTES=$(du -bs "$DIRECTORY" | cut -f1)
