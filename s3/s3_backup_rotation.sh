...
find $S3_CONFIGS_PATH -type f -name "*.cfg" | while read -r env_data; do
  ...
  log "Creating temporary S3 configuration file at $tmp_conf"
  ...
  log "Deleting files uploaded more than $S3_BACKUP_ROTATION_TIME_LIMIT days ago..."
  s3cmd -c "$tmp_conf" ls "s3://$s3_path" | grep -v " DIR " | while read -r line; do
    ...
    if [[ $create_date -le $older_than ]]; then
      if [ $file_name != "" ]; then
        log "Deleting $file_name in S3"
        s3cmd -c "$tmp_conf" del "$file_name"
        pure_file_name=$(echo "$file_name" | awk -F'/' '{ print $NF }')
        log "Also deleting local encrypted file $base_encrypt_dir/$pure_file_name if it still exists"
        rm -f "$base_encrypt_dir/$pure_file_name"
        unencrypted_file=/etc/unencrypted/$pure_file_name

        if [[ -f $unencrypted_file ]]; then
          log "Deleting $unencrypted_file if it still exists"
          rm -f $unencrypted_file
        fi
      fi
    else
      log "   File $file_name still valid"
    fi
  done
