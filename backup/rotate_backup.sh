...
rotate_directory() {
  ...
  # Delete old files
  find "$dir" -maxdepth 1 -type f -mtime +"$BACKUP_MAX_DAYS" -delete
  ...
  log "Deleted $FILES_DELETED files older than $BACKUP_MAX_DAYS days."
  log "Kept $FILES_AFTER files newer than $BACKUP_MAX_DAYS days."
